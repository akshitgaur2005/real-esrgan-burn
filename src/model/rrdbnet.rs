...
impl ResidualDenseBlockConfig {
    pub fn init<B: Backend>(&self, device: &B::Device) -> ResidualDenseBlock<B> {
        ResidualDenseBlock {
            conv1: Conv2dConfig::new([self.num_feat, self.num_grow_ch], [3, 3]).with_stride([1, 1]).with_padding(PaddingConfig2d::Explicit(1, 1)).init(device),
            conv2: Conv2dConfig::new([self.num_feat + self.num_grow_ch, self.num_grow_ch], [3, 3]).with_stride([1, 1]).with_padding(PaddingConfig2d::Explicit(1, 1)).init(device),
            conv3: Conv2dConfig::new([self.num_feat + 2 * self.num_grow_ch, self.num_grow_ch], [3, 3]).with_stride([1, 1]).with_padding(PaddingConfig2d::Explicit(1, 1)).init(device),
            conv4: Conv2dConfig::new([self.num_feat + 3 * self.num_grow_ch, self.num_grow_ch], [3, 3]).with_stride([1, 1]).with_padding(PaddingConfig2d::Explicit(1, 1)).init(device),
            conv5: Conv2dConfig::new([self.num_feat + 4 * self.num_grow_ch, self.num_feat], [3, 3]).with_stride([1, 1]).with_padding(PaddingConfig2d::Explicit(1, 1)).init(device),
...
impl RRDBNetConfig {
    pub fn init<B: Backend>(&self, device: &B::Device) -> RRDBNet<B> {
        ...
        RRDBNet {
            conv_first: Conv2dConfig::new([self.num_in_ch, self.num_feat], [3, 3]).with_stride([1, 1]).with_padding(PaddingConfig2d::Explicit(1, 1)).init(device),
            body: body,
            conv_body: Conv2dConfig::new([self.num_feat, self.num_feat], [3, 3]).with_stride([1, 1]).with_padding(PaddingConfig2d::Explicit(1, 1)).init(device),
            conv_up1: Conv2dConfig::new([self.num_feat, self.num_feat], [3, 3]).with_stride([1, 1]).with_padding(PaddingConfig2d::Explicit(1, 1)).init(device),
            conv_up2: Conv2dConfig::new([self.num_feat, self.num_feat], [3, 3]).with_stride([1, 1]).with_padding(PaddingConfig2d::Explicit(1, 1)).init(device),
            conv_hr: Conv2dConfig::new([self.num_feat, self.num_feat], [3, 3]).with_stride([1, 1]).with_padding(PaddingConfig2d::Explicit(1, 1)).init(device),
            conv_last: Conv2dConfig::new([self.num_feat, self.num_out_ch], [3, 3]).with_stride([1, 1]).with_padding(PaddingConfig2d::Explicit(1, 1)).init(device),
            lrelu: LeakyReluConfig::new().with_negative_slope(0.2).init()
        }
    }
}
